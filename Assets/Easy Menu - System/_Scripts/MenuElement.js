  ...
  switch (type)
  {
   ...
   case ElementTypes.button_LoadLevel: 
       if (GUI.Button (Rect (currentPosition.x, currentPosition.y, size.x, size.y), GUIContent (caption, icon)))
          if (!locked) 
            {
             if (parentElement.gameObject.audio) 
             {
              beenClicked = true;
              parentElement.gameObject.audio.Play();
             }
              else
            ...
            if (parentElement)
             if (parentElement.gameObject.audio && beenClicked)  
                  if (!parentElement.gameObject.audio.isPlaying) Application.LoadLevel(parameterFloat);
      break; 
